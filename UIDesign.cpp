...
void print(const std::vector<Rectangle*>& recs){
    for(int i = 0; i < recs.size(); i ++){
        std::cout << recs[i]->get_id() << " " << recs[i]->get_r() << " " << recs[i]->get_g() << " "
                  << recs[i]->get_b();
        if(i != recs.size()-1)std::cout << std::endl;
    }
}

inline double calculate_gray_val(const double r,const double g, const double b){
    return r * 0.299 + g * 0.587 + b * 0.114;
}

bool cmp_by_id(const Rectangle* rec1, const Rectangle* rec2){
    return std::stoi(rec1->get_id().substr(1)) < std::stoi(rec2->get_id().substr(1));
}

bool cmp_by_gray(const Rectangle* rec1, const Rectangle* rec2){
    double gray_val1 = calculate_gray_val(rec1->get_r(), rec1->get_g(), rec1->get_b());
    double gray_val2 = calculate_gray_val(rec2->get_r(), rec2->get_g(), rec2->get_b());
    if(gray_val1 < gray_val2)return true;
    if(gray_val1 > gray_val2)return false;
